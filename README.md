...
8. Token 自动化：新增服务器时自动生成并用于 FRPS/FRPC，降低不一致风险。
9. 回报链路增强：支持自定义回调地址、全局回调地址、多地址轮询回报与后台心跳重试。
...
- `FRP_REPORT_ONLINE_TTL`：最近回报在线判定窗口（秒，默认 90）
- `FRP_MANAGER_PUBLIC_URL`：全局回调地址（可填多个，逗号或空格分隔）

## 首次使用流程

1. 打开面板地址后会自动跳转到初始化页面。
2. 设置管理员账号和密码。
3. 新增 FRPS 服务器（无需输入服务器地址，可选填写“回调地址”），复制一键部署命令到目标子服务器执行。
...
建议：优先在服务器配置中填写“回调地址”，或设置 `FRP_MANAGER_PUBLIC_URL`，确保至少一个地址可从 FRPS 服务器访问。

3. FRPC 报错 token 不匹配  
原因：FRPS 仍在运行旧进程或旧配置。  
建议：重新执行面板生成的 FRPS 一键部署命令（脚本会自动停止旧进程并启动新配置），再生成 FRPC 命令。
