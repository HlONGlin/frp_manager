...
def _ensure_known_fields(payload, allowed_fields):
    ...
    if unknown:
        ...
        raise ValidationError(f'Unknown fields: {unknown_display}')
...
def validate_server_create(payload, local_ip):
    ...
    return {
        'name': _as_text(payload.get('name'), 'name', default='FRPS Server', pattern=NAME_PATTERN),
...
def validate_port_create(payload):
    ...
    if protocol not in PROTOCOLS:
        raise ValidationError('protocol must be one of tcp, udp, http, https')
    ...
    return {
        'name': _as_text(payload.get('name'), 'name', default='Port Mapping', pattern=NAME_PATTERN),
...
def validate_system(system):
    ...
    if normalized not in {'linux', 'windows'}:
        raise ValidationError('system must be linux or windows')
    return normalized
