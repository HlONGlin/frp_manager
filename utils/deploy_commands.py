...
def build_frpc_deploy_script(server, port, system='linux', security_profile='balanced'):
    ...
    if system == 'windows':
        ...
        ps_script = (
            ...
            f"Invoke-WebRequest -Uri '{BASE_DOWNLOAD_URL}/{WINDOWS_PACKAGE_NAME}' -OutFile $zipPath; "
            "Expand-Archive -Path $zipPath -DestinationPath $deployRoot; "
            f"$workDir=Join-Path $deployRoot '{WINDOWS_FOLDER_NAME}'; "
            "if (-not (Test-Path (Join-Path $workDir 'frpc.exe'))) { throw '解压后未找到 frpc.exe，请检查下载包或网络。' }; "
            f"$cfgPath=Join-Path $workDir '{config_name}'; "
            f"[System.IO.File]::WriteAllText($cfgPath,[System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String('{config_b64}'))); "
            "$outLog=Join-Path $workDir 'frpc-start.out.log'; $errLog=Join-Path $workDir 'frpc-start.err.log'; "
            "$frpcPath=Join-Path $workDir 'frpc.exe'; "
            "$p=Start-Process -WindowStyle Hidden -FilePath $frpcPath -WorkingDirectory $workDir -ArgumentList @('-c', $cfgPath) -RedirectStandardOutput $outLog -RedirectStandardError $errLog -PassThru; "
            "$p.Refresh(); Start-Sleep -Seconds 3; $p.Refresh(); "
            "if ($p.HasExited) { "
            "Write-Host 'FRPC 启动日志（最近 40 行）：'; "
            "if (Test-Path $outLog) { Write-Host '--- stdout ---'; Get-Content $outLog -Tail 40 | ForEach-Object { Write-Host $_ } } "
            "if (Test-Path $errLog) { Write-Host '--- stderr ---'; Get-Content $errLog -Tail 40 | ForEach-Object { Write-Host $_ } } "
            "if (-not (Test-Path $outLog) -and -not (Test-Path $errLog)) { Write-Host '未找到启动日志文件。' }; "
